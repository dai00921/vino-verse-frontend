document.addEventListener('DOMContentLoaded', () => {
    ...
    const wines = [
        { id: 'wine1', name: '星屑の囁き', description: '夜空に輝く星々のように、繊細で神秘的な味わい。', image: 'images/wine1.png' },
        { id: 'wine2', name: '太陽の抱擁', description: '情熱的な太陽の光を浴びた、力強く芳醇な香り。', image: 'images/wine2.png' },
        { id: 'wine3', name: '月の雫', description: '静寂な夜に降り注ぐ月の雫のように、優雅で穏やかな口当たり。', image: 'images/wine3.png' },
        { id: 'wine4', name: '風の歌', description: 'そよ風が運ぶメロディのように、軽やかで心地よい余韻。', image: 'images/wine4.png' },
    ];

    const questions = [
        { id: 'q1', category: 'mood', icon: '🌈', title: '今の気分は？', text: '今のあなたの気分に一番近いものを選んでください。', type: 'choice', options: ['穏やか', '活発', '神秘的', '情熱的'] },
        { id: 'q2', category: 'journey', icon: '🌅', title: '理想の旅は？', text: '心惹かれる旅のスタイルを選んでください。', type: 'choice', options: ['自然の中でのんびり', '都会で刺激的に', '歴史を感じる旅', '未知の冒険'] },
        { id: 'q3', category: 'senses', icon: '🍃', title: '好きな音は？', text: '心地よいと感じる音を選んでください。', type: 'choice', options: ['波の音', '鳥のさえずり', '都会の喧騒', '静寂'] },
        { id: 'q4', category: 'imagination', icon: '✨', title: 'どんな物語が好き？', text: 'あなたが主人公になりたい物語のジャンルを選んでください。', type: 'choice', options: ['ファンタジー', 'SF', 'ミステリー', 'ロマンス'] },
        { id: 'q5', category: 'reflection', icon: '💭', title: '未来への一言', text: '未来の自分にメッセージを送るとしたら？', type: 'text' },
    ];
    ...
    function renderQuestion() {
        ...
        questionElements.categoryIcon.textContent = question.icon;
        ...
        buttons.back.style.visibility = state.currentStep === 0 ? 'hidden' : 'visible';
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const poemData = await mockApiCall(state.userAnswers);
    ...
    function mockApiCall(answers) {
        ...
        return new Promise(resolve => {
            setTimeout(() => {
                ...
                const title = allText.includes('海') ? '潮風の記憶' : '森のささやき';
                const verses = [
                    'あなたの言葉が、風に乗り、',
                    '遠い記憶の扉を叩く。',
                    'そして、新たな物語が始まる。'
